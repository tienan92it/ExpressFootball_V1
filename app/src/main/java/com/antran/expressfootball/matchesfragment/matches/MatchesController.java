...
public class MatchesController extends RecyclerView.Adapter<MatchItemController> implements MatchesViewListener {
    ...
    public MatchesController(Context context, MatchesView matchesView, GenericRequestBuilder requestBuilder, MatchesControllerListener ltn) {
    ...
    public MatchItemController onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        MatchItemView matchItemView = new MatchItemView(parent, requestBuilder);
    ...
    public void loadMore(int current_page) {
        ...
        if (leagueId != 0) {
            MatchDataHelper.getLimitMatchesFromLeague(leagueId, LIMIT * current_page, new FindCallback<ParseObject>() {
                @Override
                public void done(List<ParseObject> objects, ParseException e) {
                    if (e == null) {
//                        matches.clear();
            ...
            MatchDataHelper.getLimitMatches(LIMIT * current_page, new FindCallback<ParseObject>() {
                @Override
                public void done(List<ParseObject> objects, ParseException e) {
                    if (e == null) {
//                        matches.clear();
 ...
 /*   private void updateItemInBackground(final MatchInfo match) {
        TeamDataHelper.getTeamFromId(match.getHomeTeam().getTeamId(), new FindCallback<ParseObject>() {
            @Override
            public void done(List<ParseObject> objects, ParseException e) {
                if (e == null && objects.size() > 0) {
                    ...
                    for (int i = 0; i < matches.size(); i++) {
                        if (matches.get(i).getMatchId() == tmpMatch.getMatchId())
                            matches.get(i).setHomeTeam(home);
                    }
                    notifyDataSetChanged();
                }
            }
        ...
        TeamDataHelper.getTeamFromId(match.getAwayTeam().getTeamId(), new FindCallback<ParseObject>() {
            @Override
            public void done(List<ParseObject> objects, ParseException e) {
                if (e == null && objects.size() > 0) {
                    Team away = new Team();
                    for (int i = 0; i < matches.size(); i++) {
                        if (match.getMatchId().compareTo(matches.get(i).getMatchId()) == 0)
                            matches.get(i).setAwayTeam(away);
                    }
                    notifyDataSetChanged();
                }
            }
        ...
        LeagueDataHelper.getLeagueFromId(match.getLeague().getLeagueId(), new FindCallback<ParseObject>() {
            @Override
            public void done(List<ParseObject> objects, ParseException e) {
                if (e == null && objects.size() > 0) {
                    ...
                    for (int i = 0; i < matches.size(); i++) {
                        if (match.getMatchId().compareTo(matches.get(i).getMatchId()) == 0)
                            matches.get(i).setLeague(league);
                    }
                    notifyDataSetChanged();
                }
            }
